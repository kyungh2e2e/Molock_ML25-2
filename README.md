...
Later sections for **scenario-based testing** are intentionally left as placeholders for teammates to fill in.
...
> File names are examples; please align them with the actual repository structure.
...
This closed-world baseline acts as a **simple, interpretable reference model**.
...
This step checks **how much we can remove purely based on correlation**, while tracking accuracy.
...
This yields a **27-feature pruned set**, where features with **essentially no contribution** (in importance) are removed.
...
### 5.6 Step 6 – Searching for a Compact Feature Set

Based on the ablation results, a small subset of candidate features to drop is chosen, for example:
...
In the final decision, one of these combinations results in **26 features**, usually by dropping a single additional feature (such as `IPI_Mean`) from the 27-feature set.
...
* **Evaluation metrics**
* **Results with 37, 27, 26 features**

### 7.2 Open-World Scenario

* **Definition of monitored vs unmonitored classes**
* **Thresholding / reject mechanism**
* **Binary vs multi-label settings**
* **Evaluation metrics (e.g., TPR, FPR, Open-Set Accuracy)**
* **Results & analysis**

### 7.3 Ablation & Sensitivity Analyses

* Performance vs number of features
* Impact of outlier cleaning
...
* The 26-feature configuration aims to balance:
  ...
  * Model simplicity
  * Interpretability
  * Performance close to (or better than) the 37-feature baseline.
